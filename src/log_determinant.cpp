...
static void log_determinant(benchmark::State& state) {
  using stan::math::var;
  ...
  auto run = [](const auto&... args) {
    return sum(log_determinant(args...));
  };
...
int end_val = 1024;
BENCHMARK(toss_me);
BENCHMARK(log_determinant)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK_MAIN();
