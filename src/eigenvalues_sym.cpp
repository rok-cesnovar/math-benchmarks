...
static void eigenvalues_sym(benchmark::State& state) {
  using stan::math::var;
  ...
  auto run = [](const auto&... args) {
    return sum(eigenvalues_sym(args...));
  };
...
int end_val = 1024;
BENCHMARK(toss_me);
BENCHMARK(eigenvalues_sym)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK_MAIN();
