...
static void mdivide_left_matrix_vector(benchmark::State& state) {
  using stan::math::var;
  ...
  auto run = [](const auto&... args) {
    return sum(mdivide_left(args...));
  };

  callback_bench_impl(init, run, state);
}

static void mdivide_left_matrix_matrix(benchmark::State& state) {
  using stan::math::var;
  using stan::math::promote_scalar;
...
int end_val = 1024;
BENCHMARK(toss_me);
BENCHMARK(mdivide_left_matrix_vector)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK(mdivide_left_matrix_matrix)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK_MAIN();
