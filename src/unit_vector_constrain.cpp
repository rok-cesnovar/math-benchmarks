...
static void unit_vector_constrain(benchmark::State& state) {
  using stan::math::var;
  ...
  auto run = [](const auto&... args) {
    return sum(unit_vector_constrain(unit_vector_constrain(args...)));
  };
...
int end_val = 1024;
BENCHMARK(toss_me);
BENCHMARK(unit_vector_constrain)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK_MAIN();
